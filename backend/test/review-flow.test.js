...
test('relay transport produces a three-hop signed hash chain', async () => {
  ...
  assert.equal(route.path.join(','), 'relay-sea-01,relay-sgp-04,relay-ams-09');
...
test('standalone relay services transport an encrypted envelope across network hops', async t => {
  ...
  const nodes = [
    { id: 'relay-sea-test', region: 'Southeast Asia', role: 'ingress', url: `http://127.0.0.1:${ports[0]}`, secret: 'sea-network-secret' },
